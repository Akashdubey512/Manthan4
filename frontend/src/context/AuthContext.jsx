...
export function AuthProvider({ children }) {
  ...
  useEffect(() => {
    async function initAuth() {
      ...
      if (error || !data) {
        localStorage.removeItem('token'); setToken(null); setUser(null); setIsAuthenticated(false);
      } else { setUser(data); setIsAuthenticated(true); }
      setIsLoading(false);
    }
    initAuth();
  }, [token]);
  ...
  const logout = async () => {
    if (token) await authService.logout().catch(() => {});
    localStorage.removeItem('token'); setToken(null); setUser(null); setIsAuthenticated(false);
  };
